...
export const Courses = () => {
  ...
  const handleClick = () => {
    setIsClicked((prev) => !prev); // toggle on each click
  };
  ...
  return (
    <section className="flex flex-col justify-center items-start py-48 pr-[10%] pl-[10%] mt-9 w-full bg-neutral-100 max-md:px-5 max-md:py-24 max-md:mt-10">
      <div className="flex flex-col -mb-10 w-full max-w-[1540px] max-md:mb-2.5 max-md:max-w-full">
        ...
        <button
          onClick={handleClick}
          className={`self-start px-16 pt-4 pb-5 mt-2 text-2xl font-semibold rounded-[52px] max-md:px-8 max-md:py-3 max-md:text-lg border transition-colors duration-300 ${
            isClicked
